...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error.code === 'unauthorized') {
      return NextResponse.json(
        { error: 'Notion access token expired. Please reconnect your workspace.' },
        { status: 401 }
      );
    }
    ...
    if (error.code === 'forbidden') {
      return NextResponse.json(
        { error: 'Insufficient permissions. Please ensure the integration has access to your pages.' },
        { status: 403 }
      );
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to fetch page content. Please try again.' },
      { status: 500 }
    );
  }
}
